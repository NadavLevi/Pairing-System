...
def test_invalid_required_location(location):
    with pytest.raises(ValueError, match="required_location must be a non-empty string"):
        ConsumerPolicy(required_location=location, required_features=["f1"], min_stake=0)
...
def test_invalid_min_stake(min_stake):
    with pytest.raises(ValueError, match="min_stake must be a non-negative integer"):
        ConsumerPolicy(required_location="US", required_features=["f1"], min_stake=min_stake)